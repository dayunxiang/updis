...
function loadGeoJSON() {
  const geojsonMap = JSON.parse(fs.readFileSync('/Users/apple/Desktop/文档/项目/深圳城市规划设计院/updis/tests/data.json'))
  const geojson = {
...
const cy = geojson2cytoscape(geojson)
...
test('测试查询地块下游管道', () => {
  ...
  const conduits = getDescendantConduitsOfSubcatchment(feature, cy)
})
...
test('测试查询地块下游排口', () => {
  ...
  const outfalls = getDescendantOutfallsOfSubcatchment(feature, cy)
...
test('测试查询排口上游管道', () => {
  ...
  const conduits = getAncestorConduitsOfOutfall(feature, cy)
})
...
test('测试查询排口上游地块', () => {
  ...
  // 查找出地块最近的点
  const subcatchmentNearestNodes = calcAllSubcatchmentNearestNode(geojson, cy)
  const subcatchments = getAncestorSubcatchmentsOfOutfall(feature, geojson, cy, subcatchmentNearestNodes)
...
test('计算地块中心点', () => {
  ...
  var result = getCenterPointOfSubcatchment(feature)
  console.log(result)
})
