<template xmlns="http://www.w3.org/1999/html">
  <div class="app-container calendar-list-container">
    ...
    <div class="filter-container">
      <el-form :inline="true">
        <el-button  style="float: right;" class="filter-item" :style="titleButtonStyle" type="primary" v-waves icon="el-icon-search" v-if="isShowSearch" @click="handleFilter">搜索</el-button>
        <!-- 过滤条件 -->
        <span style="float: right;" v-for="(filter) in filters">
        ...
        <!-- end 过滤条件 -->
        <!--自定义-->
        <span style="float: right;" v-if="definedOperate.length" v-for="(operate,index) in definedOperate" :key="index">
          <!--自定义按钮-->
          ...
          <el-form-item v-if="operate.type == 'select'" :label="operate.label">
            <el-select v-model="operate.value" :placeholder="operate.placeholder" @change="selectChange(operate.func, operate.value)" clearable>
              <el-option v-for="(o,index) in operate.options" :key="index" :label="o.label" :value="o.code"></el-option>
            </el-select>
          </el-form-item>
          <!--自定义输入框-->
          <el-form-item v-if="operate.type == 'input'" :label="operate.label">
            <el-input @keyup.enter.native="handleFilter"
                      ...
                      clearable
                      class="filter-item"
                      @focus="definedFocus(operate.focus)"
          ...
          <!--自定义导入-->
          <el-form-item v-if="operate.type == 'upload'">
            <el-upload :accept="operate.accept || '*/*'"
                       :name="operate.param || 'picture'"
                       :headers="operate.headers || null"
                       :action="operate.url || '/api/upload'"
                       :limit="operate.limit || 1"
                       :show-file-list="false"
                       :on-remove="operate.handleRemove || handleRemove"
                       :on-change="operate.change || handleChange"
                       ref="upload"
                       :on-success="operate.uploadSuccess || uploadSuccess">
              <el-button slot="trigger" size="medium" type="primary" :icon="operate.icon"
                         :disabled="operate.disabled">{{operate.label}}</el-button>
            </el-upload>
          </el-form-item>
        </span>
        <!--预定义按钮-->
        <el-button-group v-if="buttonGroup">
          ...
          <el-button class="filter-item" :style="titleButtonStyle" type="primary" v-waves icon="el-icon-close" v-if="isBatchRemove && multipleSelection.length" @click="BatchRemove">批量删除</el-button>
        </el-button-group>
        ...
        <span v-if="!buttonGroup">
          ...
          <el-button class="filter-item" :style="titleButtonStyle" type="primary" v-waves icon="el-icon-plus" v-if="isShowNewButton" @click="openDialog('newData')">新建</el-button>
          <el-button class="filter-item" :style="titleButtonStyle" type="primary" v-waves icon="el-icon-refresh" v-if="isShowRefresh" @click="refreshList">刷新</el-button>
          <span class="filter-item" v-if="isBatchRemove && multipleSelection.length">
            <el-button  :style="titleButtonStyle" type="primary" v-waves icon="el-icon-close"  @click="BatchRemove">批量删除</el-button>
          </span>
          <span v-if="singleOperate.length && multipleSelection.length" v-for="(operate,index) in singleOperate" :key="index" class="filter-item" style="margin-right: 5px;">
               <el-button v-if="operate.type=='button'"  type="primary"  :style="operate.style"  v-waves :icon="operate.icon" @click="operate.func">{{operate.label}}</el-button>
              <el-dropdown trigger="click" v-if="operate.type=='dropdown'" @command="operate.handleCommand">
                <el-button  class="el-dropdown-link" type="primary" v-waves>
                    {{operate.label}}<i class="el-icon-arrow-down el-icon--right"></i>
                </el-button>
                <el-dropdown-menu slot="dropdown">
                    <el-dropdown-item v-for="(drop,key) in operate.option" :key="key" :command="drop.status">{{drop.text}}</el-dropdown-item>
                </el-dropdown-menu>
               </el-dropdown>
          </span>
        </span>
    ...
    <!-- 表格 -->
    <el-table v-if="testTable" :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row :cell-style="cellStyle" ref="multipleTable"
    ...
    <div class="pagination-container" v-if="isShowPagination">
      <el-pagination background @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page.sync="listQuery.pageNo" ref="paginationRef"
      ...
      filterParams: function() {
        ...
        _.each(Object.keys(ret[self.schema['modelUnderscore']]), function(column) {
          const operValue = ret[self.schema['modelUnderscore']][column]
	        if ( Object.keys(operValue)[0] && Object.keys(operValue)[0].indexOf('%') >= 0) {
		        ret[self.schema['modelUnderscore']][column][Object.keys(operValue)[0]] = ret[self.schema['modelUnderscore']][column][Object.keys(operValue)[0]].replace(/%/g,"\\%")
	        }
	        if ( Object.keys(operValue)[0] && Object.keys(operValue)[0].indexOf('_') >= 0) {
		        ret[self.schema['modelUnderscore']][column][Object.keys(operValue)[0]] = ret[self.schema['modelUnderscore']][column][Object.keys(operValue)[0]].replace(/_/g,"\\_")
	        }
    ...
    methods: {
      ...
      setDefinedOperate() {
        ...
        if (self.userDefined.definedOperate) {
          ...
          if (self.userDefined.singleOperate) {
          self.singleOperate = self.userDefined.singleOperate
        }
        if (self.userDefined.definedOperation) {
          self.definedOperation = self.userDefined.definedOperation
	        self.testTable = false
          if (self.userDefined.setOperationWidth == undefined) {
            self.setOperationWidth()
          } else {
            self.operationWidth = self.userDefined.setOperationWidth
          }
	        setTimeout(function () {
		        self.testTable = true
	        },4)
      ...
      setOperationWidth() {
        const self = this
        let operationWidth = 20

        if (self.options.editData && self.options.editData.isShow === true) {
          operationWidth += 40
        }
        if (self.options.showDeleteButton === true) {
          operationWidth += 40
        }
        if (self.options.showDetail && self.options.showDetail.isShow === true) {
          operationWidth += 40
        }
        _.each(self.definedOperation, function(item, index) {
          _.each(item.label, function(value) {
            if (!isNaN(Number(value))) {
              operationWidth += 8 // 如果是数字+8
            } else {
              operationWidth += 13 // 如果是汉字加13
            }
          })
          if (self.userDefined.definedOperation.length > 1 && index > 0) {
            operationWidth += 13 // 每添加一条需加上margin
          } else {
            operationWidth += 5
          }
        })
	      // console.log('operationWidth', operationWidth)
      ...
      sortChange(row) {
      	const self = this
      	if(!row.prop){
		      self.listQuery.sortItem = self.options.sortItem
		      self.listQuery.sortOrder = self.options.sortOrder
		      self.getList()
		      return false
	      }
      ...
      getList() {
        ...
        if(self.userDefined && self.userDefined.requestParams){
          requestParams = self.userDefined.requestParams(params)
        }
        request(self.schema.modelUnderscorePlural, requestParams).then(resp => {
          let respList = []
          if (resp.data.length !== 0 && resp.data[0].superior !== undefined && resp.data[0].includes !== undefined &&
            ...
            // console.log(typeof(item.superior.params))
              try {
                var  stringObj =JSON.parse(item.superior.params)
                    // console.log(Object.prototype.toString.call(stringObj))
                    if (Object.prototype.toString.call(stringObj ) === '[object Array]') {
                            for(var key in stringObj ){
                                // console.log(stringObj[key] )
                                for(let k in stringObj[key] ){
                                    item.superior[k]=stringObj[key][k]
                             }
                        }
                    }
            } catch(e) {
          ...
          if (self.options && self.options.useTableTotal !== false) {
      ...
      openDialog(type, data) {
        ...
        if (type === 'editData') {
          ...
          self.options.editData.headers ? self.HmComplexForm.headers = self.options.editData.headers : ''
        ...
        if (type === 'newData') {
          ...
          self.options.newData.headers ? self.HmComplexForm.headers = self.options.newData.headers : ''
        ...
        if (type === 'detail') {
          ...
          self.options.showDetail.headers ? self.HmComplexForm.headers = self.options.showDetail.headers : ''
      ...
      isShowOperation(row, operation) {
        const self = this
        if (operation.isShowOperation !== undefined) {
          return operation.isShowOperation(row)
        } else {
          return true
        }
      },
      ...
      refreshList() {
        ...
        if (this.userDefined && this.userDefined.refreshList) {
          this.userDefined.refreshList()
        }
        this.init()
        this.getList()
      },
      ...
      deleteDataRequest(data) {
        ...
        self.$confirm('此操作将永久删除数据, 是否继续?', '提示', {
          ...
          request(self.schema.modelUnderscorePlural + '/delete/batch', {
            ...
            if (self.userDefined && self.userDefined.deleteCallBack) {
              self.userDefined.deleteCallBack(data)
            }
      ...
      setOptions() {
        ...
        if (self.options.isBatchRemove !== undefined) { // 判断是否显示批量删除
          self.isBatchRemove = self.options.isBatchRemove
        }
        ...
        if (self.options.HmFullCalendar) { // 显示日历组件
	  ...
	  updated(){
		  const self = this
		  if (self.$refs.paginationRef) {
			  let childrenWidth = 0
			  _.each(self.$refs.paginationRef.$vnode.elm.children, function (item) {
				  childrenWidth += item.clientWidth
			  })

			  if (childrenWidth > self.$refs.paginationRef.$vnode.elm.clientWidth) {
				  self.$refs.paginationRef.$vnode.elm.style.overflow = 'scroll'
				  self.$refs.paginationRef.$vnode.elm.style.overflowY = 'hidden'
			  }else {
				  self.$refs.paginationRef.$vnode.elm.style.overflow = 'initial'
			  }
		  }
	  }
  }
...
<style>
  ...
  .el-table .descending  .descending{
	  border-top-color: #00BF8B !important;
  }
  .el-table .ascending .ascending{
	  border-bottom-color: #00BF8B !important;
  }
