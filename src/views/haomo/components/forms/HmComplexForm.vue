      ...
      <el-col :span="layout ? layout.middle : 23">
        ...
        <el-form ref="form"
          ...
          <el-form-item v-for="column in showUserColumns"
            ...
            <!-- 2 下拉框 -->
            <span v-else-if="column.widgetType === 2">
                <el-select
                       :ref="column.ref || ''"
                       v-model="formModel[column.codeCamel]"
                       @change="column.change && column.change($event, formModel)"
            ...
            <el-button class="filter-item" type="primary"  v-if="formStyle && formStyle.select && formStyle.select.isShowBtn" :style= "formStyle.select.button.style || {width: '70%'}" icon="el-icon-setting" @click="formStyle.select.button.func">{{formStyle.select.button.text}}</el-button>
            </span>
            ...
            <el-upload v-else-if="column.widgetType === 8"
                       ...
                       :on-remove="(file, fileList)=>{ return handleRemove(file, fileList, column.codeCamel)}"
                       class="a"
                       :headers="uploadHeaders || null"
                       :limit="column.limit || 3"
                       :on-change="column.change || handleChange"
                       :file-list="column.fileList"
                       ref="upload"
                       :before-upload="(file)=>{ return beforeUpload(file, column.beforeUpload, column.codeCamel)}"
            ...
            <div class="hm-form_form_div" v-else-if="column.widgetType === 9" :style="formStyle && formStyle.elTree && formStyle.elTree.style || {width: '70%'}">
              <el-tree :data="column.options"
                       ...
                       @check-change="(data, checked, indeterminate)=>{ return handleCheckChange(data, checked, indeterminate, column.codeCamel, column.checkStrictly, column.change)}"
            ...
            <!--10 密码框-->
            <el-input v-else-if="column.widgetType === 11" :type="eyeType"
                      @change="column.change && column.change($event,formModel)"
                      :style="formStyle && formStyle.input && formStyle.input.style || {width:'70%'}"
                      :disabled="column.disabled"
                      :readonly="column.readonly"
                      v-model="formModel[column.codeCamel]">
              <el-button slot="append" icon="el-icon-view" @click="iconClick"></el-button>
              <!--<svg-icon icon-class='eye'/>-->
            </el-input>
            <!--选择其他模块文件-->
            <div class="hm-form_form_div" v-else-if="column.widgetType === 12" :style="formStyle && formStyle.elTree && formStyle.elTree.style || {width: '70%'}">
              <select-file :columns="column" :formModel="formModel"></select-file>
            </div>
            <!--地图-->
            <div v-else-if="column.widgetType === 13" class="amap-page-container" :style="formStyle && formStyle.input && formStyle.input.style || {width:'70%'}">
              <el-input v-model="formModel[column.codeCamel]"></el-input>
              <el-amap-search-box class="search-box" :on-search-result="onSearchResult" :center="center" style="width: 185px;height: 25px;"></el-amap-search-box>
              <el-amap :vid="'amap-vue'" :zoom="zoom" :plugin="plugin" :center="center" :events="{
                click: function(e) {
                  marker.position = [e.lnglat.lng, e.lnglat.lat]
                  formModel[column.codeCamel] = e.lnglat.lng + ',' + e.lnglat.lat
                }
                }" style="height: 300px;">
                <el-amap-marker :position="marker.position" :events="marker.events" :visible="marker.visible" :draggable="marker.draggable"></el-amap-marker>
              </el-amap>
            </div>
            <div v-else-if="column.widgetType === 14">
              <el-button type="primary" icon="el-icon-setting" @click="showDepartmentType">部门类型</el-button>
            </div>
            <!-- 11 普通input  || {width:'65%'}-->
...
<script>
  ...
  import selectFile from '@/views/selectFile/selectFile'
  ...
  export default {
    ...
    props: {
       ...
       * 第三个参数为当前删除的文件的详细信息, 第四个参数为当前上传文件所绑定的的表单字段
       * 属性removeFun,删除文件的回调函数，第一个参数为当前删除文件的信息file，第二个参为当前删除的文件所绑定的的表单字段，第三个参数为表单的绑定对象formModel
       ...
       *      return formModel
       *    }，
       *    uploadFun: function(response, formModel, file, code) {
       *      do something
       *    },
       *    removeFun: function(file, code, formModel) {
       *      do something
       *    }
       *  }
       */
...
//            this.formModel[column.codeCamel] = e.lnglat.lng + ',' + e.lnglat.lat
//          }
//        },
        plugin: [{ // 地图的放大缩小工具
          pName: 'ToolBar',
    ...
    created() {
      ...
      this.getData()
      this.getList()
      this.uploadHeaders = (this.headers && !this.isEmptyObject(this.headers)) ? this.headers : null
    },
    // 监听id变化从新init
    watch:{
    ...
    methods: {
      ...
      iconClick: function() {
        if (this.eyeType === 'password') {
          this.eyeType = 'text'
        } else if (this.eyeType === 'text') {
          this.eyeType = 'password'
        }
      },
      // 文件上传前的函数
      beforeUpload(file, userBeforeUpload, code) {
        if (userBeforeUpload) {
          return userBeforeUpload(file, code)
        }
        return true
      ...
      deleteUploadFile(filePath) {
        ...
        // 删除服务器上封面图片的文件
        const header = (self.headers && !self.isEmptyObject(self.headers)) ? self.headers : {}
      ...
      uploadSuccess(response, file, fileList, code, fileData) {
        ...
        if (self.funObject && self.funObject.uploadFun) {
          self.funObject.uploadFun(response, self.formModel, file, code)
        }
      },
      uploadClick1() {
      ...
      handleRemove(file, fileList, code) {
        const self = this
        console.log('文件删除', file, fileList, code, self.formModel)
        self.uploadText = '文件删除'
        ...
        if (file.response && file.response.message) {
          _.each(self.formModel[code], function(item, index) {
            if (_.endsWith(item, file.response.message)) {
              self.$delete(self.formModel[code], index)
            }
          })
        ...
        if (self.funObject && self.funObject.removeFun) {
          self.funObject.removeFun(file, code, self.formModel)
        }
      ...
      // 树形选择器
      handleCheckChange(data, checked, indeterminate, code, checkStrictly, change) {
        const self = this
        console.log('handleCheckChange函数', data, checked, indeterminate)
        // console.log(data)
        // console.log(checked)
      ...
      getData() {
        ...
        const temp = { 'Content-Type': 'application/json;charset=UTF-8' }
        const header = (self.headers && !self.isEmptyObject(self.headers)) ? _.merge(temp, self.headers) : temp
      ...
      newRelateData() {
        ...
        // 处理请求头信息
        const temp = { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }
        const header = (self.headers && !self.isEmptyObject(self.headers)) ? _.merge(temp, self.headers) : temp
        ...
        request(self.schema.modelUnderscorePlural, {
          ...
          if (resp.data.length > 0 && resp.data[0].superior !== undefined && !self.isEmptyObject(resp.data[0].superior) && resp.data[0].refers !== undefined && !self.isEmptyObject(resp.data[0].refers) && resp.data[0].includes !== undefined && !self.isEmptyObject(resp.data[0].includes)) {
            ...
            if (self.funObject && typeof (self.funObject.beforeRender) === 'function') {
              console.log('self.formModel____________:', self.formModel)
              self.formModel = self.funObject.beforeRender(resp.data, self.formModel)
            }
                ...
                transformRequest:
                  function(obj) {
                    ...
                    obj = _.omitBy(obj, function(value) {
                      return value === null || _.trim(value) === '' // 删除value=null的属性，剩下的返回给新对象
                    })
                    for (var p in obj) {
                      str.push(encodeURIComponent(p) + '=' + encodeURIComponent(obj[p]))
                    }
              ...
              request(self.schema.modelUnderscorePlural + '/new', {
                ...
                transformRequest:
                  function(obj) {
                    ...
                    obj = _.omitBy(obj, function(value) {
                      // return !value
                      return value === null || _.trim(value) === ''
                    })
                    var str = []
                    for (var p in obj) {
                      str.push(encodeURIComponent(p) + '=' + encodeURIComponent(obj[p]))
